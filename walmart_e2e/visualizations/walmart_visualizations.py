...
# ── Output folder ─────────────────────────────────────────────────────────────
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'walmart_charts')
...
FROM WALMART_FACT_TABLE f
JOIN WALMART_DATE_DIM  d ON f.DATE_ID  = d.DATE_ID
JOIN WALMART_STORE_DIM s ON f.STORE_ID = s.STORE_ID AND f.DEPT_ID = s.DEPT_ID
...
SELECT YEAR(d.STORE_DATE) AS "Year",
       SUM(f.STORE_WEEKLY_SALES) AS "Weekly_Sales"
FROM   WALMART_FACT_TABLE f JOIN WALMART_DATE_DIM d ON f.DATE_ID=d.DATE_ID
GROUP BY 1 ORDER BY 1"""
...
FROM   WALMART_FACT_TABLE f WHERE f.CPI IS NOT NULL
GROUP BY 1 ORDER BY 1"""

SQL_R10 = """
SELECT f.DEPT_ID AS "Dept", SUM(f.STORE_WEEKLY_SALES) AS "Weekly_Sales"
FROM   WALMART_FACT_TABLE f GROUP BY 1 ORDER BY 1"""
...
def main():
    print('=' * 65)
    print('  Walmart BI — Python Visualizations (Snowflake → PNG)')
    print('=' * 65)
